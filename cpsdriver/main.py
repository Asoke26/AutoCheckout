...
def generate_receipts(test_client):
    global testName
    aProduct = test_client.list_products()[-1]
    itsFacing = test_client.find_product_facings(aProduct.product_id)[0]
    logger.info("Product named '{}' is found on plate {}, at coords {}".format(
        aProduct.name, plateIdToStr(itsFacing.plate_ids[0]), itsFacing.coordinates))

    didWrite = False
    dT = 1.0/60
    obj_type = "plate_data"
    csvFile = "/app/csv/{}.csv".format(testName)
    ...
    while len(nextData) > 0:
        ...
        logger.info("Gondola: {}".format(gondolaId))
        ...
        for jj in range(nShelves):
            logger.debug("Looking at shelf {}".format(startShelf+jj))
            for ii in range(nPlates):
                logger.debug("Looking at plate {}".format(startPlate+ii))
                weightData = (rawData.data[:,jj,ii]).reshape(nSamples,1)
                if not(np.isnan(weightData).all()):
                    logger.info("Gondola {}, shelf {}, plate {}".format(gondolaId, jj,ii))
